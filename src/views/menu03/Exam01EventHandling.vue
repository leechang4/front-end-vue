    ...
    <div class="card-body">
      <div>
        ...
        <button
          ...
          name="btn2"
          @click="handleBtn2('vue is good', $event)"
        >
          버튼2
        </button>
        <button
        ...
        <form @submit.prevent="handleSubmit">
          <div class="d-flex align-items-center justify-content-center">
            <input type="sumbit" value="제출" class="btn btn-info btn-sm mr-2" />
            <button class="btn btn-info btn-sm mr-2">제출</button>
            <input type="image" :src="require(`@/assets/logo.png`)" height="30" />
          </div>
        </form>
        <hr />
        <button class="btn btn-info btn-sm mr-2" @click.ctrl="handleBtn3">ctrl+click</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handleBtn2(arg, event) {
      // arg는 vue is good이 들어어옴
      console.log("arg: ", arg);
      console.log("event: ", event);
      if (event.target.name === "btn2") {
        console.log("btn2가 클리됨");
    ...
    handleSubmit() {
      console.log("AJAX 통신으로 폼 데이터 전송");
    },
    handleBtn3() {
      console.log("handleBtn3() 실행");
    },
  },
};
</script>
